...
class TestProgramListing(ProgramsApiConfigMixin, CredentialsApiConfigMixin, SharedModuleStoreTestCase):
    ...
    def setUp(self):
        ...
        # TODO: Move the below to setUpClass? Only needs to happen once.
        ...
        run_mode = programs_factories.RunMode(course_key=unicode(self.course.id))  # pylint: disable=no-member
        course_code = programs_factories.CourseCode(run_modes=[run_mode])

        self.first_program = programs_factories.Program(
            organizations=[organization],
            course_codes=[course_code]
        )
        self.second_program = programs_factories.Program(
            organizations=[organization],
            course_codes=[course_code]
        )

        self.data = sorted([self.first_program, self.second_program], key=self.program_sort_key)

    def program_sort_key(self, program):
        """
...
class TestProgramDetails(ProgramsApiConfigMixin, SharedModuleStoreTestCase):
    ...
    def setUp(self):
        ...
        self.organization = programs_factories.Organization()
        self.run_mode = programs_factories.RunMode(course_key=unicode(self.course.id))  # pylint: disable=no-member
        self.course_code = programs_factories.CourseCode(run_modes=[self.run_mode])
        self.data = programs_factories.Program(
